...
const Steps = ({
  ...
  const submit = async () => {
    submitForCompletion(currentStepCount);
  };
  ...
  return (
    <>
      <View style={styles.bottomTimer}>
        ...
        <Text style={styles.timerFont}>{currentStepCount}</Text>
      </View>
...
export const ActiveWorkout = () => {
  ...
  const saveWorkout = async (steps: number, currentUser: string) => {
    ...
    await db().ref(`/users/${currentUser}/sessions/${sessionId}`).set({ steps, time, date: sessionId })
  };
  ...
  const sortAndSave = async (allValues: LeaderboardEntry[]) => {
    ...
    await db().ref(`/`).update({ leaderboard: Object.fromEntries(newLeaderboard) })
  }
  ...
  const updateLeaderboard = async (steps: number, currentUser: string) => {
    ...
    const leaderboardindex = allValues.findIndex((value) => value.name === userName)
